...
$id_comentario_r=$_POST['id_comentario_r'];
$id_usuario_r=$_POST['id_usuario_r'];
$respuesta=$_POST['respuesta'];
$ins = "INSERT INTO respuesta(id_comentario_r,id_usuario,respuesta)VALUES ('$id_comentario_r','$id_usuario_r','$respuesta')";
try {
...
if (isset($_SESSION['Correo'])) {
  ...
  $rec = $conn->prepare('SELECT * FROM recetas WHERE id = :id_receta ');
			...
			<div class="comments">
				...
				?>
					...
					<?php 	if (isset($_SESSION['Correo'])) {?>
								action="nuevoComentario.php" method="POST"<?php
							}else{
							?>
								action="InicioSesion.php" method=""
						<?php
			...
			<div class="container-comments">
				...
				?>
					...
					?>
						...
						<form 
					<?php 	if (isset($_SESSION['Correo'])) {?>
								action="nuevaRespuesta.php" method="POST"<?php
							}else{
							?>
								action="InicioSesion.php" method=""
						<?php
							...
							?> >
							<input name="id_receta" type="hidden" value="<?php echo $results['id']?>">
							<input name="id_comentario_r" type="hidden" value="<?php echo $row['id']?>">
							<input name="id_usuario_r" type="hidden" value="<?php echo $usuarios['id']?>">
				...
				<?php
					$Resp = "SELECT * FROM respuesta WHERE id_comentario_r=$row[id] order by id desc";
					$resRes=$conn -> query($Resp);
					while($MosResp=$resRes->fetch()){


					$sqlResU = "SELECT  id, concat(Nombre, ' ',II_nombre, ' ', Apellido, ' ', II_apellido) as nombres FROM usuarios where id=$MosResp[id_usuario]";
					$sqlUr=$conn -> query($sqlResU);
					$resSqU=$sqlUr->fetch()
		  			...
		  			<div class="comments-request">
						...
						<div class="info-comments-request">
							...
							<div class="headerc">
								...
								<h4><?php echo $resSqU['nombres'];?></h4>
								<h5>25 noviembre 2017</h5>
							</div>

							<p><?php echo $MosResp['respuesta']; ?></p>
